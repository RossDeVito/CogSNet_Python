...
def evaluate_for_node(events, surveys):
	...
	for survey_time in surveys.keys():
		...
		ordered_inds = (-np.asarray(list(canidate_most_recent.values()))).argsort()
		ordered_canidates = np.asarray(
			list(canidate_most_recent.keys()))[ordered_inds]
...
def evaluate_recency_model(edge_dict, interaction_dict, survey_dict):
	similarities = []
...
if __name__ == "__main__":
	...
	result = evaluate_recency_model(edge_dict, interaction_dict, survey_dict)

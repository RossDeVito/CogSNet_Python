...
if __name__ == '__main__':
    with open(os.path.join("data", "ml_data.pkl"), 'rb') as pkl:
    ...
    for participant_id in data:
        for edge in data[participant_id]:
            x = data[participant_id][edge][survey_time, :]
            y = labels[participant_id][edge][survey_time]
            if y > -1 and y <= 10:
                X.append(x)
                Y.append(y)

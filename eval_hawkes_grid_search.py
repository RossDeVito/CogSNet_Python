...
from rankers import HawkesRanker, HawkesRankerL
...
if __name__ == "__main__":
	...
	for train_inds, test_inds in k_fold.split(surveys):
		...
		for L in range(1, 366):
			for p in [.01, .1, .5]:
				...
				ranker = HawkesRankerL(L, p)
				ranker_res.append(
					ranker.score(interaction_dict, survey_dict_test))
				ranker_res[-1]['desc'] = str(ranker)
	...
	res_df.to_csv('hawkes_grid_search.csv')
